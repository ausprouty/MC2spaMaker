...
export default {
	...
	async addNote(noteid, route, noteText){
		var source = localStorage.getItem('mc2NoteSourceSpa');
		if (source == 'database'){
	...
	async openDatabase(){
		try {
			...
			const isConn = (await sqlite.isConnection("db_notes_spa")).result;
			//console.log(`after isConnection ${isConn}`);
			let db;
			if (ret.result && isConn) {
				//console.log("I am retreiving connection")
				db = await sqlite.retrieveConnection("db_notes_spa");
			} else {
				//console.log("I am creating  connection")
				db = await sqlite.createConnection("db_notes_spa", false, "no-encryption", 1);
			}
			await db.open();
			return db
		} catch (err) {
  ...
  // Dealing with Textarea Height
	...
	async getDataSource(){
		var noteSource = localStorage.getItem('mc2NoteSourceSpa')
		if (!noteSource){
